...
print("✅ 数据集加载完毕。")
```
【数据清洗与预处理说明】 由孟雨欣负责完成。原始的 Supplementary Dataset 2 包含了大量的 OTU 原始绝对丰度数据。为了确保后续降维算法（PCA）的准确性并减少统计噪音，我们执行了严格的数据过滤：

1.剔除无效物种：利用 otu_df.sum(axis=1) > 0 算法，对所有在 96 个根系样本中总丰度为 0 的孤立 OTU 进行了解析度剔除。

2.群落相对丰度标准化：由于各样本的原始测序深度存在差异，我们对过滤后的总 OTU 进行总和归一化（Total Sum Scaling, TSS）处理，将绝对丰度全系统转化为相对丰度（百分比），从而确保样本间具有可比性。
